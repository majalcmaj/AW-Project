...
int p2b(const char* in_path, const char* out_path) 
{
    rgb_image image;
    int res = 0;
...
{
    if(argc != 4) {
        fprintf(stderr, "Exactly 3 arguments should be provided: MODE (p2b - png to bin or b2p - bin to png) IN_FILE OUT_FILE\n");
        return 3;
    }

    const char* mode = argv[1];
    if(strcmp(mode, "p2b") == 0) {
        return p2b(argv[2], argv[3]);
